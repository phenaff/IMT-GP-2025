...
régression mais ici, il suffit de résoudre pour $\lambda_1$ 
une des deux equations suivantes:

\begin{align*}
\bar{r_A} &= \beta_A \lambda_1 \\
\bar{r_B} &= \beta_B \lambda_1 
\end{align*}
...
On vérifie que les deux equations donnent des résultats cohérents:
...
```
```{r, echo=FALSE}
names(lambdas) <- c('equation 1', 'equation 2')
kable(lambdas, booktabs=TRUE, col.names="$\\lambda$", escape=FALSE, digits=3)
```
