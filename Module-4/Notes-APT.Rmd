...
On construit $(N-K)$ portefeuilles $v_i, i=1, \ldots, N-K$, orthogonaux entre eux et orthogonaux aux colonnes de $B$. Soit $v_{i,j}$ le poid de l'actif $j$ dans le portefeuille $i$. Sans perte de généralité, on pose $\sum_j v_{i,j} = 1$. Finalement, définissons le portefeuille $v$, moyenne des portefeuilles précédents:

\begin{align}
v = \frac{1}{N-K} \sum_{i=1}^{N-K} v_i
\end{align}

Soit $a = \max |v_i|, i=1, \ldots, N-K$. Avec des portefeuilles suffisament diversifiés, $a$ est $O(1/N)$. Calculons la variance du rendement de ce portefeuille moyen:

\begin{align}
\Var{v^Tr} &= \Var(v^TBf + v^T \epsilon) \\
&= \frac{1}{(N-K)^2} Var((\sum_i v_i )^T \epsilon) \\
&\leq \frac{1}{(N-K)^2} (N-K) N a^2 \sigma^2 \\
&\leq \frac{1}{N(N-K)} \sigma^2
\end{align}
